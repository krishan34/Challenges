...
const printGoals = function (...player) {
  console.log(`${player.length} goals scored by ${player}`);
};
...
team1 < team2 &&
  console.log(`${game.team1} is more likely to win then ${game.team2}`);
team1 > team2 &&
  console.log(`${game.team2} is more likely to win then ${game.team1}`);
...
let average = 0;
for (const values of Object.values(game.odds)) average += values;
...
// const newDogJuliaList = dogsJulia.slice(1, 3);
// console.log(newDogJuliaList, dogsJulia);

// 2. Create an array with both Julia's (corrected) and Kate's data

// const dogList = newDogJuliaList.concat(dogsKate);
// console.log(dogList);
...
const dAge1 = [16, 6, 10, 5, 6, 1, 4];
const dAge2 = [5, 2, 4, 1, 15, 8, 3];

const calcAverageHumanAge = function (ages) {
  const humanAge = ages.map((age) => (age <= 2 ? 2 * age : 16 + age * 4));
  const adult = humanAge.filter((age) => age >= 18);
  const calAvgAge = Math.round(
    adult.reduce((acc, age) => acc + age, 0) / adult.length
  );
  return calAvgAge;
};
const avg1 = calcAverageHumanAge(dAge1);
const avg2 = calcAverageHumanAge(dAge2);
...
const calcAverageHumanAge = function (ages) {
  const humanAge = ages
    ...
    .filter((age) => age >= 18)
    // only way to calculate avg by dividing age evertime with lenght.
...
console.log(
  `Sarah Dog is Eating too ${
    sarahDog.curFood > sarahDog.recFood ? "Much" : "Liitle"
  }`
);
